...
- `domain/contracts/data/todo-operational.contract.yaml` — raw JSONL product (operational
  awareness): `{event_type, todo_id, user_id, timestamp}`. Deliberately excludes the todo title
  and any email — no PII in the long-retention layer. Storage location
  `s3://todo-data/todo-operational/**/*.jsonl`.
- `domain/contracts/data/todo-performance.contract.yaml` — curated Parquet product
  (created-vs-completed per day): `{date, event_type, count}`. Storage location
